...
LATITUDE = 45.07
LONGITUDE = 7.54
FILE_LAST_HOUR = "ultima_ora_icon_ch2_hail.txt"
...
def estrai_limiti_run(hourly_data: dict, ref_param: str) -> tuple[bool, str, datetime]:
    ...
    if os.path.exists(FILE_LAST_HOUR):
        ...
        if ultima_ora_valida_str <= ultima_ora_salvata:
            print(f"✅ Run ICON-CH2 Grandine {nome_run} già elaborato.")
            return False, "", None
...
def genera_album_grandine(dt_run_utc: datetime, nome_run: str):
    rome_tz = pytz.timezone("Europe/Rome")
    ...
    # Colori ottimizzati per la grandine (da turchese/blu a giallo/arancio/rosso scuro) in cm
    my_levels = [0.5, 1, 1.5, 2, 3, 4, 6, 8, 10]
    my_colors = ["#41b6c4", "#1d91c0", "#225ea8", "#253494", "#fed976", "#fd8d3c", "#f03b20", "#bd0026"]
    ...
    for block_name, ore_list in blocchi.items():
        print(f"\nGenerazione album Grandine: {block_name}")
        
        # Per la grandine non serve l'ora precedente
        ...
        try:
            tot_hail = ogd_api.get_from_ogd(req)
            # Scenario peggiore tra gli 11 membri
            hail_stat = tot_hail.max(dim="eps") 
            # Se preferisci la media, scommenta questa e commenta la riga sopra:
            # hail_stat = tot_hail.mean(dim="eps")
        ...
        for h in ore_list:
            # ICON DWD/MeteoSwiss fornisce la grandine in metri. Moltiplichiamo per 100 per avere i CM.
            hail_step = hail_stat.sel(lead_time=np.timedelta64(h, 'h')) * 100

            prec_geo = regrid.iconremap(hail_step, destination)
            ...
            title = f"ICON-CH2 EPS (Scenario Peggiore) - Max Grandine (cm)\nRun: {dt_run_utc.strftime('%d/%m/%Y %H:%M UTC')} | {str_valida}"
            chart.title(title)
            chart.legend(label="Dimensione Grandine (cm)")

            filename = f"hail_{h}.png"
            chart.save(filename)
            percorsi_foto.append(filename)
            
            plt.close(chart.fig)
        
        caption_album = f"🧊 ICON-CH2 EPS: Rischio Grandine\n🗓 {block_name}\n⚙️ Run {nome_run}"
        invia_album_telegram(percorsi_foto, caption_album)
        
        for f in percorsi_foto:
            if os.path.exists(f): os.remove(f)
        del tot_hail, hail_stat
        time.sleep(15)

def main():
    print("Cerco l'ultimo run completo ICON-CH2 via Open-Meteo per la grandine...")
    ...
    if is_new:
        print(f"🚀 Lancio generazione Album Grandine per il RUN {nome_run} ({dt_run_utc})")
        genera_album_grandine(dt_run_utc, nome_run)
    else:
